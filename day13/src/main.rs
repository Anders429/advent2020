...
fn subsequent(busses: &[Bus]) -> usize {
    let mut x = 0;
    let mut n = match busses[0] {
        Bus::InService(val) => val,
        _ => 0,
    };
    for (i, val) in busses[1..busses.len()]
    .iter()
    .enumerate()
    .filter(|(i, bus)| matches!(bus, Bus::InService(_)))
    .map(|(i, bus)| if let Bus::InService(val) = bus {(i, *val)} else {(i,0)}) {
        println!("{} {} {}", x, n, val);
...
fn subsequent2(busses: &[Bus]) -> usize {
    let operating = busses
        .iter()
        .enumerate()
        .filter(|(_, bus)| matches!(bus, Bus::InService(_)))
        .map(|(i, bus)| {
    ...
    let mut diff_found = true;
    let mut vals = operating
        .iter()
        .cloned()
        .map(|(_, val)| val)
        .collect::<Vec<_>>();
    ...
    loop {
        ...
        for i in 0..vals.len() {
            if vals[i] < start {
                vals[i] = operating[i].1 - (start % operating[i].1);
                if i == 0 {
                    start = vals[i];
                }
            }
            if (vals[i] - operating[i].0) % operating[0].1 != 0 {
                diff_found = true;
            } 
        }
    }
